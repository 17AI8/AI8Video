...
- 传尾帧串联新增自动与手动两种模式：自动模式连续推进，手动模式先传入上一条视频的最新尾帧并展示预览，只有点击“继续”才提交下一条；“刷新尾帧”可重新读取上一条成片，不触发生成；
- 服务重启或任务中断后，若已有成片仍可复用，系统会恢复为手动等待状态，并按当前会话批次与本地成品库对账，避免旧批次失败记录覆盖已生成结果；
- 结果卡明确区分准备尾帧、等待继续、已提交和生成中状态，未取得任务 ID 的视频不会显示为已经提交；
- 视频裁剪时间轴修复版本冲突与拖拽回弹：发生并发更新时自动携带最新 revision 重提本次编辑，同时保留用户刚刚拖动的边界；
- 恢复链路继续沿用原规划台词并正常生成后续 TTS，不会主动覆盖用户已经手工修改的既有配音结果。
...
|---|---|---|---|
| 用户素材、项目状态和最终结果默认保存在本机 | 从热点、知识、脚本到生成、精剪和归档是一条完整链路 | Skill 提供策略，类型化 Capability 约束输入输出、取消、生命周期和副作用顺序 | 视频、TTS 与 HTML 动效采用非破坏性三轨时间轴，保留源边界并在确认后烧录 |
...
- TTS 时间轴支持试听、拖动、删除、切分、左右边缘非破坏性裁剪或恢复、按停顿智能切块和独立重生成；
- 当前草稿可通过系统“保存为”对话框导出 `192 kbps` MP3；
- 视频、TTS 与 HTML 动效组成三条独立编辑轨，共享刻度尺、播放时钟、播放头、剪刀参考线和吸附提示；
...
- 三轨草稿保存 `schemaVersion`、`revision` 和原始源边界，既能恢复被裁掉的内容，也能阻止旧页面覆盖较新的编辑；
...
编辑阶段始终保留纯净基础视频和三条独立时间轴。视频与 TTS 编辑会刷新真实候选预览；HTML 动效先更新 WAAPI 实时预览，只有渲染输入变化时才重建透明层。确认前系统统一检查裁剪后的视频边界，确认后依次完成视频裁剪、TTS、背景音乐和 HTML 透明层烧录，并原子替换正式视频；任一步失败都不会把半成品标记为已应用。
