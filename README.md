...
| **多 Agent 协作** | Supervisor（监督者）统一编排 Intent Agent（意图判断）、Planner（规划者）与 Reviewer（审核者），使用本地任务图、租约和账本管理并发、取消与恢复。 |
| **批量任务** | 统一管理生成批次、任务进度、报告、告警与真实额度保护；失败视频支持单条重试，缺失首帧时可自动重建。 |
| **配音、字幕与合并** | 通过本地 TTS、FFmpeg 完成配音、字幕、背景音乐和媒体后处理；结果库支持按勾选顺序批量合并并清理源资产。 |
| **HTML 动效** | 使用项目自有 WAAPI（Web Animations API）时间线生成动效，支持实时叠加预览、逐 Chunk 调整时间轴，确认前不修改正式视频。 |
...
- **确认后落盘**：HTML 动效实时预览和时间轴调整只更新候选结果，用户点击确认烧录后才替换正式视频。
...
AI8video 采用**本地有界多 Agent**，不是让多个通用 Agent 任意调用工具。当前由 Supervisor 负责统一调度，Intent Agent、Planner 与 Reviewer 作为受约束的专业角色执行路由、规划和审核；任务状态持久化在本地 SQLite 账本中，并通过依赖关系、租约、心跳、重试和取消规则保证恢复一致性。模型与媒体能力仍由现有 Python Runtime 调用，不拆成独立微服务。
