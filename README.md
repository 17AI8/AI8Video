...
- 模型设置合并为统一主标签，并按文本／视频规划、多模态、图片和视频模型提供侧栏子标签；每类模型支持保存多套折叠配置、复制副本和单选启用，API Key 留空保存时保持原值；
- 智能分集的重新规划、确认和取消改由模型结构化理解用户原文，不再依赖脆弱的固定文本规则；确认后的规划直接进入生成，模型返回数量异常时保留明确错误而不伪造分集内容；
- 手动传尾帧恢复会保护已有远端任务 ID，不再把仍在提交、轮询或归档的任务提前改成“等待继续”；单条重试保留完整批次结果卡，并以本地归档资产恢复前序完成态；
- 视频延长的四份批量修图与视频生成使用独立状态文案：修图、视频生成和远端完成但结果待同步不再互相混淆，刷新与局部重绘不会重置未变化的结果卡；
- 花字烧录与图片水印烧录改为独立开关，关闭花字不会再连带关闭水印；任务失败卡压缩空白区域，取消按钮保持在文字右侧；
- 视频预览升级为画面、TTS、HTML 动效与背景音乐四层独立结构：各层共享播放时钟但保持独立真值，背景音乐不再提前混入候选视频，确认烧录时才统一合成；
- 批量合并会同步合并源视频对应的视频片段、TTS、HTML 动效和背景音乐轨道；合并结果继续保留可编辑分段，而不是只留下一个不可拆分的最终 MP4；
- TTS 与 HTML 动效时间轴支持框选、组合选择、批量拖动和批量删除；点击 TTS chunk 可切换当前项，点击轨道空白处可解除选择，指针捕获结束后仍可继续拖拽；
- 合并预览按源片段坐标恢复 TTS 与 HTML 动效编辑记录，避免片段被错误压缩为 0.1 秒；已保存的 TTS 时间轴在合并视频时长存在轻微漂移时仍可恢复；
- 批量候选按单项任务状态即时刷新图片和视频结果，选择、清理、尾帧续接与合并操作不再因轮询重绘而丢失交互状态；
- 视频延长与批量修图状态可在刷新后恢复并续接远端轮询，未确认的四份候选不会自动折叠为单一结果；提示词必填，额外参考图仅作为可选增强输入；
- 重新生成默认继承原视频提示词；AI 扩写以当前有效文字数为基准逐次增加约 10 字，字数统计不包含标点；
- 结果弹窗、工作台通知、爆款拆解折叠列表、选择框、进度文案和预览操作按钮完成一轮紧凑化与状态可读性修复。
