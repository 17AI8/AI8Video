...
| **迭代批量生成** | 普通聊天单轮可生成 1～5 条独立的 10 秒视频，按“生成 → 审核成片 → 优化下一条”串行执行。 |
...
1. 单轮允许生成 `1～5` 条独立视频，每条固定为 `10` 秒。
2. 即使全局配置为并发或选择 `merge2` / `merge4`，该模式仍使用直接串行流水线，不自动拼接成一条长视频。
3. 第 N 条完成并归档后，系统会抽取多个时间点组成联系表，审核真实成片中的问题、优化空间和下一条约束。
4. 审核结论只优化第 N+1 条，不自动重做当前视频，避免无限循环和重复计费。
5. 当前一条生成、归档或成片审核不可用时，后续尚未提交的视频会停止并标记为跳过，避免在失去反馈后继续消耗额度。

`AI8VIDEO_DRY_RUN=1` 下的生成与审核会明确标记为模拟结果，不会伪装成真实审核通过。每日定时跑量使用的 `DailyBatchRunner` 保持原有语义，不受上述普通聊天单轮上限影响。
...
├─ generation/    脚本拆分、生成流水线、迭代批量与结果审核
