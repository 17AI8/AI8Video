...
| **配音、字幕与合并** | 通过本地 TTS、FFmpeg 完成配音、字幕、背景音乐和媒体后处理；结果库支持按勾选顺序批量合并并清理源资产。 |
| **视频预览精剪** | 在预览阶段裁剪视频、编辑 TTS 波形切块并调整 HTML 动效时间轴；越界片段会标红并阻止烧录，确认前不修改正式视频。 |
| **HTML 动效** | 使用项目自有 WAAPI（Web Animations API）时间线生成动效，支持实时叠加预览、逐 Chunk 调整时间轴，确认后统一烧录配音与动效。 |
| **热点与爆款拆解** | 聚合公开热点源；爆款拆解按“宫格画面 → 台词识别 → 镜头语言 → 猜剧本”顺序解锁，并将分析结果沉淀为可复用创作证据。 |
...
- **结果修订**：视频预览中可编辑续作提示词并单条或批量延长；配音台词可结合知识库进行 AI 润色、扩写，并由 Reviewer 按视频时长和字数复核。
- **预览精剪**：裁剪视频后仍可继续调整 TTS 与动效；超过新视频结尾的片段会标红，只有将其剪除或拖回有效范围后才能确认烧录。
- **Reviewer 兜底**：HTML 动效每轮方案都会给出 0～100 分；通过即采用，达到最大尝试次数仍未通过时，自动选择最高分方案并保留简短说明。
- **确认后落盘**：视频裁剪、TTS 波形和 HTML 动效的所有修改都先作为候选预览，用户点击确认烧录后才统一替换正式视频。
...
- **爆款拆解**：先使用 Whisper 识别台词，再从最多 10 个代表帧提取镜头语言；猜剧本复用台词与镜头证据，不重复执行旧版画面分析。
...
| `用户文件夹/HTML动效/reviews/` | HTML 动效候选预览、时间轴数据和待确认烧录文件 |
