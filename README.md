...
| **配音、字幕与合并** | 通过本地 TTS、FFmpeg 完成配音、字幕、背景音乐和媒体后处理；TTS 波形支持按停顿智能切块，结果库支持按勾选顺序批量合并并清理源资产。 |
...
- **精剪工具**：视频胶片提供定位和剪刀工具，三条时间轴保持单选；TTS 可根据音波停顿智能切块。超出新视频结尾的片段只在对应时间轴标红，处理完越界片段后才允许确认烧录。
