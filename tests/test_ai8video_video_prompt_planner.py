...
class AI8VideoVideoPromptPlannerTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_planning_prompt_requires_independent_complete_videos(self) -> None:
        prompt = build_video_planning_prompt("老板在会议室讲封号风险", 4, "商务真实感")

        self.assertIn("规划 4 条彼此独立、可分别发布的短视频提示词", prompt)
        self.assertIn("每条视频都必须自成一体", prompt)
    ...
    def test_keyword_extraction_uses_llm_structured_result(self) -> None:
        ...
        self.assertEqual(guidance["video_keyword_guidance"][0]["keywords"], ["AI8video"])

    def test_planning_prompt_receives_model_keyword_guidance(self) -> None:
        guidance = {
            "global_keywords": ["AI8video", "AI8VIDEO", "6月18日"],
            "must_preserve_facts": ["AI8videoAI8VIDEO全球发布"],
            "video_keyword_guidance": [
                {
                    ...
                    "keywords": ["私域承接"],
                    "facts": ["客户关系沉淀"],
                    "usage_note": "放在第二条视频口播中自然承接",
                }
            ],
            "usage_policy": "按独立视频角度自然使用，不硬塞。",
        }

        prompt = build_video_planning_prompt("长剧本正文", 2, keyword_guidance=guidance)
    ...
    def test_planning_prompt_receives_task_constraints(self) -> None:
        prompt = build_video_planning_prompt(
            "老板在办公室讲私域痛点",
            2,
    ...
    def test_planner_runs_keyword_model_before_planning_model(self) -> None:
        model_inputs = []

        def fake_llm(prompt: str) -> str:
            model_inputs.append(prompt)
            if "素材关键词理解模型" in prompt:
                return """
                {
        ...
        videos = plan_video_prompts_with_ai("AI8video AI8VIDEO 6月18日发布", 1, llm=fake_llm)

        self.assertEqual(len(model_inputs), 2)
        self.assertIn("素材关键词理解模型", model_inputs[0])
        self.assertIn("文本模型提取的关键词指导", model_inputs[1])
        self.assertEqual(videos[0].keyword_guidance["preserved_keywords"], ["AI8video", "AI8VIDEO"])
        self.assertIn("global", videos[0].keyword_guidance)

    def test_planner_repairs_invalid_json_array_once(self) -> None:
        model_inputs = []
        ...
        videos = plan_video_prompts_with_ai("2个，消息", 1, llm=fake_llm)
    ...
    def test_planning_prompt_requires_dialogue_and_shootable_segments(self) -> None:
        ...
        self.assertIn("每条视频必须先规划可直接口播/对白的中文台词", prompt)
    ...
    def test_planning_prompt_can_plan_against_final_merged_duration(self) -> None:
        prompt = build_video_planning_prompt(
            "按四个镜头讲一个完整热点事件",
            1,
    ...
    def test_rewrite_prompt_keeps_dialogue_requirement(self) -> None:
        video = VideoPrompt(index=1, title="封号危机", prompt="老板在办公室焦虑", source_summary="")
        prompt = build_rewrite_prompt(video, "台词更狠一点", "商务真实")
    ...
    def test_planning_prompt_allows_user_requested_poster_text_style(self) -> None:
        prompt = build_video_planning_prompt("老板在会议室讲封号风险", 2, "老板真实口播、大量大字报式文字")
    ...
    def test_planner_does_not_locally_normalize_brand_or_append_old_fidelity_notes(self) -> None:
        ...
        self.assertIn("飞讯发布预热", videos[0].title)
        self.assertIn("台词/口播：飞讯", videos[0].prompt)
        self.assertNotIn("信息保真", videos[0].prompt)
        self.assertNotIn("口播保真", videos[0].prompt)
        self.assertNotIn("618 倒计时 5 天", videos[1].prompt)

    def test_planner_does_not_append_protection_notes_to_titles(self) -> None:
        def fake_llm(_prompt: str) -> str:
            return """
            [
        ...
        self.assertEqual(videos[0].title, "沟通噩梦")
        self.assertNotIn("品牌保真", videos[0].title)
        self.assertNotIn("信息保真", videos[0].title)
        self.assertNotIn("品牌保真", videos[0].prompt)
        self.assertNotIn("信息保真", videos[0].prompt)

    def test_planner_does_not_parse_business_prompt_as_local_forbidden_terms(self) -> None:
        ...
        self.assertIn("全球发布倒计时", videos[0].prompt)
        self.assertNotIn("最终硬性约束", videos[0].prompt)
        self.assertNotIn("用户可编辑业务模型系统提示词", videos[0].prompt)

    def test_planning_prompt_keeps_dates_as_spoken_by_default_not_visible_text(self) -> None:
        prompt = build_video_planning_prompt(
            "老板口播 6月18日全球发布倒计时",
            2,
            None,
    ...
    def test_single_prompt_preserves_brand_and_ai8video_terms(self) -> None:
        video = single_prompt_to_video("AI8video AI8VIDEO 618 倒计时 5 天，老板口播私域承接", None)[0]

        self.assertIn("AI8video", video.prompt)
        self.assertIn("AI8VIDEO", video.prompt)
        self.assertIn("618 倒计时 5 天", video.prompt)
    ...
    def test_rewrite_does_not_locally_normalize_brand_or_restore_countdown(self) -> None:
        ...
        rewritten = rewrite_video_with_ai(video, "语气更狠一点", llm=fake_llm)
    ...
    def test_planning_prompt_tells_model_not_to_invent_unrequested_identity_details(self) -> None:
        prompt = build_video_planning_prompt("一个人顶一个团队", 1)
