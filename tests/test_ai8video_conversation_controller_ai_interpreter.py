...
class AI8VideoConversationControllerAiInterpreterTest(unittest.TestCase):
    ...
    def _build_result() -> PipelineResult:
        request = ParsedRequest(raw_text="商务风三个10s", mode="multi_episode_script")
        episode = EpisodePrompt(index=1, title="单条视频", prompt="商务风三个10s")
        job = QuickVideoJob(
            episode_index=1,
    ...
    def test_merge_message_prefers_ai_interpretation_for_freeform_count_and_topic(self) -> None:
        ...
        self.assertEqual(state.draft.mode, "multi_episode_script")
        self.assertEqual(state.draft.episode_count, 10)
    ...
    def test_rewrite_request_uses_ai_intent_and_instruction(self) -> None:
        ...
        pipeline.rewrite_episode.return_value.to_dict.return_value = {
            "episodes": [{"index": 2, "title": "第 2 集", "prompt": "新版"}],
            "jobs": [],
            "outcomes": [],
            "archives": [],
            "assetRecords": [],
        ...
        self.assertEqual(reply.meta["rewrittenEpisodeIndex"], 2)
        self.assertEqual(reply.meta["rewriteInstruction"], "老板表情更坚定，语气更像真实开会")
        pipeline.rewrite_episode.assert_called_once()
