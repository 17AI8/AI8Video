    ...
    function placeVideoPreviewPopover(popover) {
      ...
      const offset = Math.max(0, body.getBoundingClientRect().bottom - controls.getBoundingClientRect().top);
    ...
    function closeVideoPreviewTtsEditor() {
      ...
      popover?.classList.remove('is-open');
      const button = els.videoPreviewBody?.querySelector('[data-video-preview-action="edit-tts-text"]');
      button?.setAttribute('aria-expanded', 'false');
      if (button) button.title = '展开台词编辑器';
    }
    ...
    async function openTtsNarrationEditorFromVideoPreview(userGeneratedKey, triggerButton = null) {
      const key = String(userGeneratedKey || '').trim();
      ...
      if (triggerButton) triggerButton.title = '收起台词编辑器';
      setTtsStatus('正在读取台词');
      if (textarea) {
        textarea.value = '';
